...
public class TurnoController {
    ...
    public ResponseEntity<Turno> registrarTurno(@RequestBody Turno turno) {
      if(pacienteService.buscar(turno.getPaciente().getId())==null ||
              odontologoService.buscar(turno.getOdontologo().getId())==null) throw new RuntimeException();
...
/*
        ...
        if (pacienteService.buscar(turno.getPaciente().getId()).isPresent() &&
            ...
            //System.out.println("paciente y odontologo existen");
            response = ResponseEntity.ok(turnoService.registrarTurno(turno));

        else
            response = ResponseEntity.status(HttpStatus.BAD_REQUEST).build();
            System.out.println("paciente y odontologo NO existen");
        return response;
*/
